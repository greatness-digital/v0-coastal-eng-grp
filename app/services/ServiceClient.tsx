...
/**
 * Mounts the shared CEG service experience for the given service key.
 * Kept as a client component so the route (page.tsx) can stay a server
 * component and export per-page metadata + JSON-LD for SEO.
 */
export default function ServiceClient({ serviceKey }: { serviceKey: string }) {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        // Tell the shared service template which page to render.
        ;(window as any).__CEG_SERVICE = serviceKey
    ...
    <>
      {error && (
        <div role="alert" style={{ padding: 24, margin: 24, border: "1px solid #b00020",
          ...
          <strong>Failed to mount service page:</strong> {error}
        </div>
      )}
      <div id="root" />
    </>
